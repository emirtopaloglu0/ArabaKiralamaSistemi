...
namespace AracKiralama
{
    public partial class AracGuncelleme : Form
    {
        ...
        private void AracGuncelleme_Load(object sender, EventArgs e)
        {
            try
            {
                id = A_Araclar.instance.carId;
                MarkaBox.SelectedIndex = MarkaBox.FindStringExact($"{A_Araclar.instance.marka.Trim()}");
                MarkaBox_SelectedIndexChanged(this, EventArgs.Empty);
                ModelBox.SelectedIndex = ModelBox.FindStringExact($"{A_Araclar.instance.model.Trim()}");
                CatBox.SelectedIndex = CatBox.FindStringExact($"{A_Araclar.instance.kategori.Trim()}");
                YakitBox.SelectedIndex = YakitBox.FindStringExact($"{A_Araclar.instance.yakit_turu.Trim()}");
                VitesBox.SelectedIndex = VitesBox.FindStringExact($"{A_Araclar.instance.vites_turu.Trim()}");
        ...
        private void UpdateCar_Btn_Click(object sender, EventArgs e)
        {
            try
            {
                DialogMesjalar.instance.UpdateCar();
                if (DialogMesjalar.instance.res == DialogResult.Yes)
                {
                    ...
                    {
                        if (Arac.AracClass.DurumGuncelle(Convert.ToInt32(Id_Label.Text),
